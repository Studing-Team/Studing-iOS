...
final class BookmarkCollectionViewCell: UICollectionViewCell {
    ...
    private let associationLabel = UILabel()
...
extension BookmarkCollectionViewCell {
    func configureCell(forModel model : BookmarkModel) {
        associationLabel.text = "\(model.association)"
...
private extension BookmarkCollectionViewCell {
    ...
    func setupHierarchy() {
        self.addSubviews(associationLabel, titleLabel, contentsLabel, postDayLabel, bookmarkImage)
    }
    
    func setupLayout() {
        associationLabel.snp.makeConstraints {
            $0.top.equalToSuperview().offset(17.25)
            $0.horizontalEdges.equalToSuperview().inset(15)
        }
        
        titleLabel.snp.makeConstraints {
            $0.top.equalTo(associationLabel.snp.bottom).offset(10)
            $0.horizontalEdges.equalToSuperview().inset(15)
        }
