...
private extension UserInfoSignUpViewController {
    func bindViewModel() {
        ...
        let input = UserInfoSignUpViewModel.Input(
            userId: userIdTitleTextField.textPublisher,
            userPw: userPwTextField.textPublisher,
            confirmPw: comfirmPwTitleTextField.textPublisher.eraseToAnyPublisher(),
            duplicateIdTap: duplicateIdButton.tapPublisher,
        ...
        output.userIdState
            ...
            .sink { [weak self] state in
                ...
                case .normal, .invalid, .duplicate, .select:
                    self?.duplicateIdButton.isEnabled = false
                    self?.duplicateIdButton.backgroundColor = .black30
                case .success, .validSuccess:
                    self?.duplicateIdButton.isEnabled = true
